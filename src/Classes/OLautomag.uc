...
function bool HandlePickupQuery( inventory Item )
{
  ...
  if (Item.class == class)
  {
    ...
    P.ReceiveLocalizedMessage( PickupMessageClass, 0, None, None, item.Class );}
...
{
  if (slavemag!=none&&slavemag.isinstate('clientnewclip'))
  return false;
...
ignores Fire, AltFire;
 ...
 begin:
  ...
  If (39<ammotype.ammoamount&&!isslave)                   //complex: gotta make sure we have enough ammo to fill the clips....
  ClipCount = 0;
...
state ClientNewclip              //reload 'em client's........
{
  ...
  simulated function AnimEnd()
  {
    if (newclipanim==0)
    Playeject();
    else if (newclipanim==1)
    Playdownclip();
    else if (newclipanim==2){
    if (!isslave&&slaverequestreload)
    slaverequestreload=false;
...
if (isslave||role==role_authority)        //do not do a tick on the slave....
return;
if (slavemag==none){ //simple reload
If (Playerpawn(owner)!=None&&(PlayerPawn(Owner).bExtra3!=0)&&(20-clipcount<AmmoType.AmmoAmount)&&(clipcount!=0)&&bcanclientfire&&!(Isinstate('clientfiring'))&&!Isinstate('clientaltfiring')&&!Isinstate('clientnewclip'))   //play the anim and goto the state
Gotostate('clientnewclip');}
else{ //complex reload...
...
Gotostate('clientnewclip');
mastertostate=true;}
else if (Playerpawn(owner)!=None&&(PlayerPawn(Owner).bExtra3!=0)&&(40-(clipcount+slaveclipcount)<ammotype.ammoamount)&&(slaveclipcount!=0)&&bcanclientfire&&!(Isinstate('clientfiring'))&&!Isinstate('clientaltfiring')&&!Isinstate('clientnewclip')){
if (!mastertostate)
slavemag.gotostate('clientnewclip');
else
slaverequestreload=true;}
...
return;
if (slavemag==none){ //simple reload
If (((Playerpawn(owner)!=None&&(bwantreload||playerpawn(owner).bextra3!=0))||(Playerpawn(owner)==none&&Pawn(Owner).enemy==None))&&(20-clipcount<AmmoType.AmmoAmount)&&(clipcount!=0))   //play the anim and goto the state
Gotostate('newclip');}
else{ //complex reload...
If (((Playerpawn(owner)!=None&&(bwantreload||playerpawn(owner).bextra3!=0))||(Playerpawn(owner)==none&&Pawn(Owner).enemy==None))&&(40-(clipcount+slaveclipcount)<ammotype.ammoamount)&&(clipcount!=0)){
Gotostate('newclip');
mastertostate=true;}
if (((Playerpawn(owner)!=None&&(bwantreload||playerpawn(owner).bextra3!=0))||(Playerpawn(owner)==none&&Pawn(Owner).enemy==None))&&(40-(clipcount+slaveclipcount)<ammotype.ammoamount)&&(slaveclipcount!=0)){
if (!mastertostate)
slavemag.gotostate('newclip');
else
slaverequestreload=true;}
